...
if (isset($data->UserName) and isset($data->Password) and isset($data->role)) {
    if (in_array($data->role, LOGGABLE_ENTITIES)) {
        ...
        if ($result->num_rows > 0) {
            ...
            echo json_decode(new HttpResult("Logged in succesfully", 200));
        } else {
            echo json_decode(new HttpResult("Wrong credentials", 200));
        }
    } else {
        echo json_decode(new HttpResult("Role: " . $data->role . " does not exists", 200));
    }
} else {
    echo json_decode(new HttpResult("Not all fields are filled in correctly", 203));
}
